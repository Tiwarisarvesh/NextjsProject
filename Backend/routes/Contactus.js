...
router.post( "/insertData",
  ...
  async (req, res) => {
    try {
      const { name , email , number , goal , comment} = req.body;
...
router.put("/updateData/:id", async (req, res) => {
  ...
  try {
    ...
    const result = await Contactus.findByIdAndUpdate(req.params.id , {$set: newData } ,{ new: true})
    res.json({result});
...
router.delete("/deleteData/:id" , async (req, res) => {
  try {
     ...
     let note = await Contactus.findById(req.params.id);
    ...
    //  }
     ...
     note = await Contactus.findByIdAndDelete(req.params.id)
