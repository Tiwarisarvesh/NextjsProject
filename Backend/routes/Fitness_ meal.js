...
router.get("/getData", async (req, res) => {
  try {
    ...
    const getData = await Fitness_meal.find({});
    res.json(getData);
  } catch (error) {
...
router.get('/getbyid/:id', async (req, res, next) => {
  try {
      const { id } = req.params;
      const data = await Fitness_meal.findOne({ _id: id });
  ...
  try {
    // Create a newNote object
     ...
     if(name){
      ...
      const data = await Fitness_meal.findById(req.params.id);
      if(!data) { 
    ...
    const result = await Fitness_meal.findByIdAndUpdate(req.params.id , {$set: newData } ,{ new: true})
    res.json({result});
...
router.delete("/deleteData/:id" , async (req, res) => {
  try {
     ...
     let note = await Fitness_meal.findById(req.params.id);
    ...
    //  }
     ...
     note = await Fitness_meal.findByIdAndDelete(req.params.id)
