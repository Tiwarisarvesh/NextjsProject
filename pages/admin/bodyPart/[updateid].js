...
function index() {
  ...
  const getDetails = async () => {
    try {
      const res = await fetch( `http://localhost:4000/api/fitnessBodypart/getbyid/${id}`, {
          method: "GET",
  ...
  const handleClick = async (e) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:4000/api/fitnessBodypart/updateData/${id}`,
        {
          method: "PUT",
          headers: {
      ...
      if (res.status === 200) {
        setName("");
        toast.success("Bodypart Updated successfully");
        history.back();
      } else {
