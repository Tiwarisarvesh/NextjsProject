...
function index() {
  ...
  const handleDelete = async(_id)=> {
    ...
    try {
      const res = await fetch( `http://localhost:4000/api/fitnessexcerise/deleteData/${_id}`, {
          method: "DELETE",
          headers: { "Content-Type": "application/json", },
        }
      );
                ...
                <div className="accent-gray">
                  <div className="row pt-4">
                    ...
                    <div className="col-xl-3">
                       <Link href="/admin/exerices/addNew">
                    ...
                    <tbody>
                      {meals1.map((meal, index) => (
                        <tr>
                          ...
                          <td> 
                             {(meal.image === "image URL") ? "image URL " : 
                            <Image
                          fill="true"
                            src={meal.image}
                            width={50}
                          ...
                          <td>{meal.name}</td>
                          <td>{meal.equipment.name}</td>
                          <td>{meal.date}</td>
                          <td>
                           <Link href={`/admin/exerices/${meal._id}`}><a><EditIcon sx={{ color: pink[500] }} /> </a></Link>
                          </td>
                          <td>
