...
function index() {
  ...
  const handleDelete = async(_id)=> {
    ...
    try {
      const res = await fetch( `http://localhost:4000/api/fitnessmeal/deleteData/${_id}`, {
          method: "DELETE",
          headers: { "Content-Type": "application/json", },
        }
      );
                    ...
                    <tbody>
                      ...
                      {mealData.map((meal, index) => (
                        <tr>
                          ...
                          <td>{meal.date}</td>
                          <td>
                           <Link href={`/admin/meal/${meal._id}`}><a><EditIcon sx={{ color: pink[500] }} /> </a></Link>
                          </td>
                          <td>
